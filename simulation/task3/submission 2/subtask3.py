...
# Maintains the whole simulation logic
...
# Main function which runs the complete code:
def main():
    if len(sys.argv) < 6:
        print("Please enter the input in following format: python subtask3.py <mean-interarrival-time> <service-time: yes/value> <mean-retransmit-time> <buffer-size: yes/value> <number-of-repetition>")
    else:
        ...
        if serviceTime == 'yes':
            stVal = input("Please input the list data: [Format for number input: '<1 2 3 4>'] ")
            serviceTimeList = [int(x) for x in stVal.split()]
            ...
            btVal = input("Please input the list data: [Format for number input: '<1 2 3 4>'] ")
            bufferSizeList = [int(x) for x in btVal.split()]
            iterData = True if input("Do you want to print the 50 iteration data? (yes/no) ") == "yes" else False
